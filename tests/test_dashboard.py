...
def test_a_cancellation_is_marked_as_made_by_sophia(conn):
    ...
    view = dashboard.team_view(conn, MONDAY_10)
...
def test_a_call_summary_is_built_from_what_happened(conn):
    agent = SophiaAgent(conn, now=MONDAY_10, client=_Client())
    ...
    summary = dashboard.summarise_call(agent, MONDAY_10, MONDAY_10, "the caller")
    ...
    calls = dashboard.team_view(conn, MONDAY_10)["calls"]
    assert calls[0]["caller"].startswith("Priya Sharma")


def test_a_call_where_nothing_was_said_is_not_recorded(conn):
    agent = SophiaAgent(conn, now=MONDAY_10, client=_Client())
    dashboard.record_call(conn, dashboard.summarise_call(agent, MONDAY_10, MONDAY_10, "the caller"))
...
def test_the_view_has_five_working_days_and_urgent_messages_first(conn):
    ...
    later = SophiaTools(conn, now=MONDAY_10)
    ...
    assert dashboard.team_view(conn, MONDAY_10)["stats"]["urgent_messages"] == 0
...
def test_a_same_day_problem_with_nothing_booked_needs_follow_up(conn):
    ...
    dashboard.record_call(conn, summary)
    assert dashboard.team_view(conn, MONDAY_10)["stats"]["follow_ups"] == 1
