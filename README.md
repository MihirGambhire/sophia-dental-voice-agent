...
| Text to speech | Cartesia `sonic-3.6`, a British voice, sympathetic and slightly slower | Chosen by listening over every Deepgram voice. One free account's credits last about a dozen calls, so several accounts are set (`CARTESIA_API_KEY_2` and on): a call moves to the next account when one runs out, and to Deepgram `aura-2-pandora-en` only when all are empty. While testing, `TTS_PROVIDER=deepgram` keeps those credits for the demo |
